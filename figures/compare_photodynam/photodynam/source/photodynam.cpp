...
int main(int argc, char * argv[]) {
  ...
  for (int i= 0; i < N-1; i++) {
    cout << "# " << i << "\t|\t" << mj[i] << "\t" << a[i] << "\t" << 2*M_PI*sqrt(a[i]*a[i]*a[i]/mj[i]) << "\t" << e[i] << "\t" << inc[i] << "\t" << om[i] << "\t" << ln[i] << "\t" << ma[i] << endl;
  }
  ...
  if (in.is_open()) {
	...
	case 'F':cout << occultn(state.getBary(),radii,u1,u2,flux,N) << "\t"; break;
	case 'x':
	  // for (int j = 0; j < N; j++) cout << state.X_LT(j) << "\t" << state.Y_LT(j) << "\t" << state.Z_LT(j) << "\t";
	  for (int j = 0; j < N; j++) cout << state.X_B(j) << "\t" << state.Y_B(j) << "\t" << state.Z_B(j) << "\t";
	  break;
	case 'v':
	  // for (int j = 0; j < N; j++) cout << state.V_X_LT(j) << "\t" << state.V_Y_LT(j) << "\t" << state.V_Z_LT(j) << "\t";
	  for (int j = 0; j < N; j++) cout << state.V_X_B(j) << "\t" << state.V_Y_B(j) << "\t" << state.V_Z_B(j) << "\t";
	  break;
